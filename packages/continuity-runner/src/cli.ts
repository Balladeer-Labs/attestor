...
if (!command || !packagePath) {
  console.error(
    "usage: continuity-runner <scaffold|seal|validate-package|qualify|qualification-request|run-one|run-one-target|run-all|run-manifest-target|exercise|offboarding> <package-or-draft-path> [control|manifest|metadata] [output]",
  );
  process.exit(2);
}
try {
  ...
  let manifest: ReturnType<typeof validateExecutionManifest> | undefined;
  if (command === "run-manifest" || command === "run-manifest-target") {
    ...
    manifest = validateExecutionManifest(
  ...
  if (command === "validate" || command === "validate-package") {
    ...
    const selected =
      command === "run-manifest" || command === "run-manifest-target"
    ...
    if (qualificationMode)
      for (const item of selected) await assertPackageReady(item);
    ...
    if (
      ...
      throw new Error(
        "baseline requires passing good, bad, refactor, and actual material-tamper controls",
      );
    console.log(JSON.stringify(output));
  } else if (command === "offboarding") {
    const exported = createOffboardingExport(pkg);
    const destination = auxiliary ?? outputPath;
    if (destination) await writeFile(destination, JSON.stringify(exported, null, 2) + "\n", "utf8");
