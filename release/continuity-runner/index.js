 ...
 */
export async function qualificationAlreadyCompleted(pkgInput, metadataInput, completedInput, executionRoot = process.cwd()) {
    const pkg = validatePackage(pkgInput);
    const metadata = validateQualificationMetadata(metadataInput);
    if (!Array.isArray(completedInput) || completedInput.length > 1000)
        return false;
    let matched = false;
    try {
        for (const entry of completedInput) {
            ...
            if (typeof entry.promiseId !== "string" || !/^prom_[a-z0-9]{8,64}$/.test(entry.promiseId))
                return false;
            if (entry.promiseId === pkg.promise.id &&
